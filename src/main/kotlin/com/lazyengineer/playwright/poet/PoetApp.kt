...
object PoetApp {
    ...
    val convertedNameProperties = listOf(
        ...
        Pair("exactName", String::class.asTypeName()),
    ...
    fun main(vararg param: String) {
        ...
        val byAriaRoleFunctions = createByAriaRoleFunctions(originalSpec)

        val optionalParamsFunctions = createOptionalParamsFunctions(originalSpec.funSpecs + byAriaRoleFunctions)

        val generatedClassName = "GeneratedLocatorImpl"
    ...
    private fun createByAriaRoleFunctions(originalSpec: TypeSpec): List<FunSpec> {
        val byRoleFunctionNamePostfix = "ByRole"
        return originalSpec.funSpecs.filter { it.name.endsWith(byRoleFunctionNamePostfix) && it.parameters.size == 2 }
            .flatMap {
    ...
    private fun applyAnyNameAndExactPropertyConversion(origOptionsMemberNameAndReturnTypes: List<Pair<String, TypeName>>): Pair<List<Pair<String, TypeName>>, Boolean> {
        ...
        return Pair(optionsMemberNameAndReturnTypes + nameMemberNameAndReturnTypes, optionsContainsNameAndExact)
    }
    ...
    private fun getOptionsStatement(
        ...
        val existingBodySuperCall = if (funSpec.body.toString().contains("super")) funSpec.body.toString() else null
        val superLine = if (existingBodySuperCall != null) {
            ...
            """
                try {
                    ${returnSnippet}super.${funSpec.name}(${funSpec.parameters.joinToString() { it.name }})
                    } catch (e: Throwable) {
